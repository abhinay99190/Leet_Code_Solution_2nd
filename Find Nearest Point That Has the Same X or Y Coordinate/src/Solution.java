...
given an array points where each points[i] = [ai, bi] represents that a point exists at (ai, bi). A point is valid if it
...
public class Solution {
    public static int nearestValidPoint(int x, int y, int[][] points) {
        int diff = 0;
        int max = Integer.MAX_VALUE , result = -1;
        for (int i = 0; i < points.length; i++) {
            if(x==points[i][0]||y==points[i][1]){
                diff = Math.abs(x-points[i][0]) + Math.abs(y-points[i][1]);
                if (max!=Math.min(max,diff)){
